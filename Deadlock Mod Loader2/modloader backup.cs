    ...
    public class ModLoader
    {
        ...
        public bool InstallDroppedFile(string filePath)
        {
            ...
            try
            {
                ...
                {
                    // For RAR files, we need to use a different extraction method
                    // Since .NET doesn't have built-in RAR support, we'll try using WinRAR or 7-Zip if available
        ...
        {
            // Remove parentheses and other problematic characters, replace spaces with underscores
        ...
        private IEnumerable<string> EnumGroupFiles(string pakPrefix, bool disabled, string folderName)
        {
            ...
            else
            {
                ...
                {
                    // Make sure we don't pick up disabled files that happen to start with the same prefix
        ...
        public void ActivateMod(ModInfo modToActivate)
        {
            ...
            // Find all disabled files for this mod
            ...
            // Extract pak prefixes from the disabled files
            var prefixes = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
            foreach (var df in disabledFiles)
            {
                string fileName = Path.GetFileName(df);
                // Remove the disabled prefix and mod separator to get the tail
            ...
            // Move files from disabled to active
        ...
        public void InitialSetup()
        {
            try
            {
                ...
                if (!originalContent.Contains("citadel/addons") || !originalContent.Contains("AddonConfig"))
                {
                    string modifiedContent = ModifyGameInfoContent(originalContent);
                    if (modifiedContent != originalContent)
                    {
                        WriteFileWithoutBOM(gameInfoPath, modifiedContent);
                        MessageBox.Show("gameinfo.gi has been successfully updated for mod support!", "Setup Complete");
                    }
                }
            }
            catch (Exception ex)
        ...
        private string ModifyGameInfoContent(string content)
        {
            ...
            if (!fsMatch.Success) return content;
            int fsStart = fsMatch.Index;
            int braceCount = 0;
            int fsEnd = -1;
            for (int i = fsStart; i < content.Length; i++)
            {
            ...
            if (fsEnd == -1) return content;
            string fsBlock = content.Substring(fsStart, fsEnd - fsStart);
            string beforeFs = content.Substring(0, fsStart);
            string afterFs = content.Substring(fsEnd);
            var spMatch = Regex.Match(fsBlock, @"SearchPaths\s*\{", RegexOptions.IgnoreCase);
            string newFsBlock;
            if (spMatch.Success)
            {
                int spStart = spMatch.Index;
                braceCount = 0;
                int spEnd = -1;
                for (int i = spStart; i < fsBlock.Length; i++)
                {
                    if (fsBlock[i] == '{') braceCount++;
                    else if (fsBlock[i] == '}')
                    {
                        braceCount--;
                        if (braceCount == 0)
                ...
                if (spEnd != -1)
                {
                    string beforeSp = fsBlock.Substring(0, spStart);
                    string afterSp = fsBlock.Substring(spEnd);
                    string newSearchPaths = GetDesiredSearchPaths();
                    newFsBlock = beforeSp + newSearchPaths + afterSp;
                }
                else
                {
                    newFsBlock = fsBlock;
            ...
            {
                int lastBrace = fsBlock.LastIndexOf('}');
                if (lastBrace > 0)
                {
                    string beforeBrace = fsBlock.Substring(0, lastBrace);
                    string afterBrace = fsBlock.Substring(lastBrace);
                    string searchPaths = GetDesiredSearchPaths();
                    newFsBlock = beforeBrace + "\n\t" + searchPaths + "\n" + afterBrace;
                }
                else
                {
                    newFsBlock = fsBlock;
            ...
            string result = beforeFs + newFsBlock + afterFs;
            if (!result.Contains("AddonConfig"))
            {
                string addonConfig = "\nAddonConfig\n{\n    \"UseOfficialAddons\" \"1\"\n}\n";
                result = beforeFs + newFsBlock + addonConfig + afterFs;
            }
            return result;
        }

        private string GetDesiredSearchPaths()
        {
            return @"SearchPaths
    {
        Game                citadel/addons
